  ...
  private middleware(): void {
    ...
    this.express.use((req, res, next) => {
      ...
      res.on("finish", () => {
        ...
        const params = [id, startTime, endTime, (endTime.getTime() - startTime.getTime()), req.socket.bytesRead, res.statusCode];
        this.pool
          .connect()
          .then(client => {
            return client
              .query('INSERT INTO requests(request_id, start_time, end_time, duration, size, status) VALUES ($1, $2, $3, $4, $5, $6)', params)
              .then(res => {
                client.release();
              })
              .catch(e => {
     ...
     .get("/dashboard/errors", (req, res, next) => {
      ...
      .get("/dashboard/health", (req, res, next) => {
        this.pool
            ...
            .then(client => {
              client.release();
              // If we get to this point we've succesfully connected to the database
